...
def set_experiment_permission(experiment_path):
    ...
    status = requests.get(url + "/api/2.0/workspace/get-status", params={"path": experiment_path},
    ...
    if "object_id" not in status:
        ...
        permissions = requests.patch(f"{url}/api/2.0/permissions/experiments/{status['object_id']}", json=params,
                                     headers=headers)
...
def init_experiment_for_batch(demo_name, experiment_name):
    ...
    requests.post(f"{url}/api/2.0/workspace/mkdirs",
...
class CVModel(pl.LightningModule):
    ...
    def __init__(self, num_classes: int = 2, learning_rate: float = 2e-4, momentum: float = 0.9,
